...
const TableInsertModal = ({ isOpen, onClose, onInsert }: TableInsertModalProps) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Tabelle einfügen">
      <div className="space-y-4">
        <div className="flex gap-4">
          <div className="flex-1">
            <label className="block text-xs font-medium text-text-primary mb-2 uppercase tracking-wide">Zeilen</label>
          ...
          <div className="flex-1">
            <label className="block text-xs font-medium text-text-primary mb-2 uppercase tracking-wide">Spalten</label>
            <div className="input flex items-center gap-1 px-2">
        ...
        <div className="flex items-center justify-end space-x-2 pt-2">
          ...
          <button onClick={() => { if (canInsert) onInsert(String(rows), String(cols)); }} className={clsx('btn', !canInsert && 'text-text-muted pointer-events-none')}>Einfügen</button>
        </div>
      </div>
    </Modal>
  );
