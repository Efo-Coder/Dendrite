...
export default function ElevatedToolbar({ disabled = false }: ElevatedToolbarProps) {
  ...
  const {
    ...
    fontPickerPos, setFontPickerPos, showTimerModal,
    saveSelection, closeAllPopups: closeContextPopups, openTimerModal,
    formatText,
  ...
  const page0 = (
    <>
      ...
      {btn((e) => {
          if (fontPickerPos) { closeContextPopups(); return; }
          closeAll(); saveSelection();
          const r = (e.currentTarget as HTMLButtonElement).getBoundingClientRect();
          const tb = floatingBarRef.current?.getBoundingClientRect();
          setFontPickerPos({ x: r.left + r.width / 2, top: tb?.top ?? r.top, bottom: tb?.bottom ?? r.bottom, left: tb?.left, width: tb?.width });
        }, 'Font', <Type className="w-4 h-4" />, !!fontPickerPos, isInCode)}
      {btn(
        (e) => {
