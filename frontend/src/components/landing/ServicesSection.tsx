  ...
  const handleMouseEnter = (ev: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (charsRef.current.length > 0) {
      tl.fromTo(
        charsRef.current,
        { y: 0 },
        { y: -32, duration: 0.15, ease: "sine.out", stagger: { each: 0.01, from: "start" } },
        0
      ).to(
        charsRef.current,
        { y: 0, duration: 0.2, ease: "sine.inOut", stagger: { each: 0.01, from: "start" } },
        0.15
      );
    }
  };
    ...
    <div ref={itemRef} className="relative overflow-hidden border-t border-foreground/10">
      ...
      >
        ...
        >
          ...
          <svg className="w-8 h-8 md:w-12 md:h-12 text-background" fill="none" stroke="currentColor" viewBox="0 0 24 24">
